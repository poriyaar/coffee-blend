            ...
            <div class="row">
                ...
                <div class="col-lg-6 product-details pl-md-5 ftco-animate">
                    ...
                    <form action="{{ route('add.cart', $product) }}" method="POST">
                        ...
                        @if ($checkingInCart == 0)
                            <button type="submit" name="submit" class="btn btn-primary btn-outline-primary">Add
                                to Cart</button>
                        @else
                            <button style="background-color: black"
                                class="text-white btn btn-warning py-3 px-5 disabled">Added to Cart</button>
                        @endif
                    </form>
                </div>
            </div>
                ...
                @foreach ($relatedProducts as $relatedProduct)
                    <div class="col-md-3">
                        <div class="menu-entry">
                            <a href="{{ route('product.single', $relatedProduct) }}" class="img"
                                style="background-image: url({{ asset('assets/images/' . $relatedProduct->image) }});"></a>
                            <div class="text text-center pt-4">
                                <h3><a
                                        href="{{ route('product.single', $relatedProduct) }}">{{ $relatedProduct->name }}</a>
                                </h3>
                                <p>
                                    {{ $relatedProduct->description }}
                                </p>
                                <p class="price"><span>${{ $relatedProduct->price }}</span></p>
                                <p><a href="{{ route('product.single', $relatedProduct) }}"
